...
public class AdminIndexSelectService {
	...
	public Admin selectTodayData(Time_now now, Admin admin) {
		...
		try {
			...
			System.out.println("오늘의 유기동물수 : " + admin.getTodayPuppy());
		} catch (IOException e) {
	...
	// 2.4 update된 게시글,방문자,가입자 의 시간을 가져옴 2분전에 새글 올라옴 이런식
	//미구현~~
	/**
	 * Admin selectNewData() AdminDao selectNewUserTime, selectNewVisitTime, selectNewPostTime
	 * 
	 * @param Time_now
	 *            now
	 * @param Admin
	 *            admin
	 * @return Admin admin : newPostTime, newVisitTime, newUserTime,
	 */
	/*
	public Admin selectNewData(Time_now now, Admin admin) {
		System.out.println("2.3 AdminIndexSelectService selectNewData()");
		// 2.4.1 새로 업로드된 데이터의 시간을 select하여 가져옴
		//AdminDao selectNewUserTime, selectNewVisitTime, selectNewPostTime
		System.out.println("2.4.1 새로 업로드된 데이터의 시간을 select하여 가져옴");
		AdminDao.getInstance().selectNewUserTime(now, admin);
		
		AdminDao.getInstance().selectNewVisitTime(now, admin);
		
		AdminDao.getInstance().selectNewPostTime(now, admin);
	...
	*/
	...
	public List<Admin> selectDataJson(Admin admin) {
		System.out.println("2.4 AdminIndexSelectService List<Admin> selectDataJson()");
